...
public class EnemyCommonManager : MonoBehaviour
{
    ...
    void Searching()//プレイヤーが一定距離にいるかを索敵。
    {
        ...
        else if(ds >= 25)//25m離れていると普通の速さで追跡する(ブースター無しで追いかけてくる。)
        {
            HoverChaser();
            //Debug.Log("通常追跡");
        }
    ...
    void Combat()//プレイヤーと戦闘する。複数の行動を無作為に行う。
    {
        ...
        {
            //Random.InitState(System.DateTime.Now.Millisecond);//シード値設定。不必要と判断。
    ...
    void BoostChaser()//通常の追跡。プレイヤーを追いかける。
    {
    ...
    void HoverChaser()//高速追跡。プレイヤーに近づく。
    {
        if (chasengTime <= 5.0f)//五秒間は前方方向へ移動する
        {
            chasengTime += 1.0f * Time.deltaTime;
    ...
    void burstFire()//プレイヤーの方向に素早い5連射撃を行う。
    {
        Quaternion lookRotation = Quaternion.LookRotation(playerTF.transform.position - transform.position, Vector3.up);
